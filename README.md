...
Utilitaire Windows qui garde ta présence Teams en **vert** en simulant une
micro-activité système à intervalle régulier. Fenêtre graphique soignée +
icône dans la barre des tâches (tray), état persistant.
...
2. **Micro-mouvement souris** — un déplacement d'1 px aller-retour, invisible.

Les deux cumulées rendent l'astuce quasi infaillible : si une appli ignore le
clavier, le mouvement souris la rattrape, et inversement. Aucun Alt+Tab, donc
pas de vol de focus pendant que tu travailles.

## Interface

Double-clique sur `bin/FuckTeamsStatus.exe` (après compilation). Une fenêtre à
onglets s'ouvre, avec un **bandeau d'état** en haut (vert = « Actif », gris =
« En pause »).
...
- **Empêcher la mise en veille de l'ordinateur** : tant que le maintien tourne,
  ...
  peut toujours s'éteindre. L'option suit l'état du maintien : si le maintien
  est en pause, l'anti-veille est relâchée.

### Onglet « Planning » (mode planifié)

Permet de déclarer des **plages horaires** pendant lesquelles le maintien
...
- Chaque plage : heure **De / à** (`HH:MM`) + jours de la semaine cochables
  (**L M M J V S D**), et un **switch d'activation** par plage. Bornes en
  intervalle `[début, fin[`.
- **Ajouter / supprimer** des plages librement.
- Quand le mode planifié est désactivé, on revient au contrôle manuel pur.
...
L'icône tray (près de l'horloge) reflète l'état (verte/grise). Clic droit :
...
- **Instance unique** : relancer l'exe alors qu'une instance tourne déjà ne crée
  pas de doublon — ça ramène simplement la fenêtre existante au premier plan.
...
Le fichier est tolérant à un BOM UTF-8 en tête (ajouté par certains éditeurs).
...
Le binaire est produit dans `bin/FuckTeamsStatus.exe`, autonome (aucune
installation de Go/.NET requise sur la machine cible). Pour lancer les tests :
...
```powershell
$env:CGO_ENABLED = "1"
go build -ldflags "-H=windowsgui -s -w" -o bin/FuckTeamsStatus.exe ./cmd/fuckteamsstatus
```
